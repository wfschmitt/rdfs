            ...
            row.each do |file|
              ...
              Net::HTTP.start(uri.host, uri.port) do |http|
                if (updated != 0) && (deleted == 0)
                  # UPDATE
                  begin
                    res = http.post_form('api_call' => 'add_query', 'filename' => filename, 'sha256sum' => sha256sum)

                    if res.body.include?('EXISTS')
                      req.set_form_data('api_call' => 'add_dup', 'filename' => filename, 'sha256sum' => sha256sum)
                    ...
                    req.set_form_data('api_call' => 'add', 'filename' => filename, 'sha256sum' => sha256sum, 'content' => file_contents)
                    res = http.request(req)
                    if res.body.include?('OK')
                      clear_update_flag(filename)
                    end
                    ...
                    @logger.warn {'add: Unable to connect to node at IP ' + ip + '.'}
                    sleep 1
                end
              ...
              begin
                ...
                if response.body.include?('OK')
                    ...
                    @logger.warn {'del: Unable to connect to node at IP ' + ip + '.'}
                    sleep 1
              end
            end
          end
